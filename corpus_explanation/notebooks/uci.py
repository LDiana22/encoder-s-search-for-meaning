...
Original file is located at
    https://colab.research.google.com/drive/1mW-afz6Z6PFtJXoyCwBQ9dL6336GGhlX
"""
...
# VECTOR_CACHE = "drive/My Drive/Thesis/.vector_cache"
# UCI_PATH = "drive/My Drive/Thesis/uci"
# IMDB_PATH ="drive/My Drive/Thesis/.data/aclimdb"
# PREFIX_DIR = "drive/My Drive/Thesis/experiments/imdb"
# MODEL_MAPPING = "drive/My Drive/Thesis/experiments/models_mappings/imdb"

VECTOR_CACHE = "../.vector_cache"
UCI_PATH = "../.data/uci"
IMDB_PATH = "../.data/imdb/aclImdb"
PREFIX_DIR = "experiments/imdb"
MODEL_MAPPING = "experiments/model_mappings/imdb/"
...
_extract_date("2020-03-13_14-02-15_e2")
...
class UCIDataset:
  ...
  def _load_data(self, text_field, label_field, path):
    ...
    for file_name in files:
        ...
        torch.manual_seed(0)
        train_subset, test_subset, valid_subset = utils.random_split(data.Dataset(sample, fields), (train,test, valid), random_state=random.seed(0))
...
# explanations = RakeMaxWordsPerInstanceExplanations(f"test-{experiment.config['max_words_dict']}", dataset, experiment.config)
...
model = MLPGen(f"softmax-yake", MODEL_MAPPING, experiment.config, dataset, explanations)
...
formated_date = start.strftime(DATE_FORMAT)
#-{experiment.config['max_words_dict']}
model = MLPGen(f"softmax-textrank", MODEL_MAPPING, experiment.config, dataset, explanations)
...
formated_date = start.strftime(DATE_FORMAT)
#-{experiment.config['max_words_dict']}
model = MLPGen(f"softmax-tfidf", MODEL_MAPPING, experiment.config, dataset, explanations)
...
for i in range(1,6):
    ...
    model = MLPGen(f"softmax-inst-rake-corpus-max-{experiment.config['max_words_dict']}", MODEL_MAPPING, experiment.config, dataset, explanations)
...
# model = MLPGen(f"test-{experiment.config['max_words_dict']}", MODEL_MAPPING, experiment.config, dataset, explanations)
# experiment.with_data(dataset).with_dictionary(explanations).with_model(model).run()
