...
#                 f.write("\n".join([f"{review} ~ {text_expl[review]}" for review in text_expl.keys()]))
    ...
    def forward(self, text, text_lengths, expl_file=None):
        ...
        expl_activ = nn.Dropout(0.1)(expl_activ)
...
model = MLPGen(f"mlp2l2hid-dropout-gen_rake_class_cleaned_{args.d}", MODEL_MAPPING, experiment.config, dataset, explanations)
