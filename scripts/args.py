...
def parse_args():
    ...
    parser.add_argument('--weight_decay', type=float, default=5e-06, help='L2 norm penalty [default: 1e-3]')
    ...
    # data
    parser.add_argument('--dataset', default='full_beer', help='choose which dataset to run on. [default: news_group]')
    ...
    parser.add_argument('--selection_lambda', type=float, default=.001, help="y1 in Gen cost L + y1||z|| + y2|zt - zt-1| + y3|{z}|")
    parser.add_argument('--continuity_lambda', type=float, default=0, help="y2 in Gen cost L + y1||z|| + y2|zt - zt-1|+ y3|{z}|")
